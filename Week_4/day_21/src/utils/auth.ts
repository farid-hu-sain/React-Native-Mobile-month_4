// src/utils/auth.ts - FIXED FULL VERSION
...
export const authUtils = {
  ...
  // Login method using Axios service dengan fallback
  ...
  // === FITUR BARU: Login dengan Fallback ===
  async loginWithFallback(username: string, password: string): Promise<User> {
    try {
      console.log('🔐 Attempting login with fallback...');
      ...
      console.log('🔄 API login failed, using demo fallback');
      
      // Fallback ke demo login
      this.demoLogin();
      const userData = this.getCurrentUser();
      
      if (!userData) {
        throw new Error('Login failed and fallback also failed');
      }
  ...
  // === FITUR BARU: Safe Login ===
  async safeLogin(username: string, password: string): Promise<User> {
    try {
      return await this.loginWithFallback(username, password);
    } catch (error: any) {
      console.error('❌ All login methods failed:', error);
      
      // Ultimate fallback - create basic user
      ...
      this.setToken(fallbackUser.token, fallbackUser);
      return fallbackUser;
    }
  },
  ...
  async loginWithTokenManagement(username: string, password: string): Promise<User> {
    try {
      ...
      if (!tokenSaved || !expiredAtSaved) {
        console.warn('⚠️ Token storage failed, but login successful');
      }
