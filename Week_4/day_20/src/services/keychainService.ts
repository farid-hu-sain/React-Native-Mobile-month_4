...
export const keychainService = {
  ...
  // Simpan access token secara secure
  async saveAccessToken(token: string): Promise<boolean> {
    try {
      return await this.setCredentials('access_token', token, KEYCHAIN_SERVICES.USER_TOKEN);
    } catch (error) {
  ...
  async getAccessToken(): Promise<string | null> {
    try {
      const credentials = await this.getCredentials(KEYCHAIN_SERVICES.USER_TOKEN);
      return credentials ? credentials.password : null;
    } catch (error) {
  ...
  async deleteAccessToken(): Promise<boolean> {
    try {
      return await this.resetCredentials(KEYCHAIN_SERVICES.USER_TOKEN);
    } catch (error) {
