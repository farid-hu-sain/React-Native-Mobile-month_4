// src/services/deepLinkingService.ts - ENHANCED VERSION WITH LOCATION
...
export class DeepLinkingService {
  ...
  private validateDeepLinkParams(parsedUrl: { action: string; params: any }): { 
    ...
    switch (action) {
      ...
      // ✅ NEW: Validasi untuk location-based deep links
      case 'toko-terdekat':
      case 'tracking-kurir':
      case 'hitung-ongkir':
        // Tidak butuh parameter khusus untuk location features
        break;
    }
  ...
  // TAMBAH: Enhanced navigation deep link handling dengan LOCATION FEATURES
  private async handleNavigationDeepLink(parsedUrl: any): Promise<boolean> {
    ...
    try {
      ...
      switch (action) {
        ...
        // ✅ NEW: Location-based deep links
        case 'toko-terdekat':
        case 'nearby-stores':
          this.navigationRef.navigate('NearbyStores', params);
          console.log('📍 Navigated to nearby stores');
          break;
          
        case 'tracking-kurir':
        case 'courier-tracking':
          this.navigationRef.navigate('CourierTracking', params);
          console.log('🚚 Navigated to courier tracking');
          break;
          
        case 'hitung-ongkir':
        case 'calculate-shipping':
          this.navigationRef.navigate('NearbyStores', { 
            ...params,
            autoCalculateShipping: true 
          });
          console.log('📦 Navigated to shipping calculation');
          break;
  ...
  private async handleFallbackDeepLink(parsedUrl: any): Promise<boolean> {
    ...
    switch (action) {
      ...
      case 'keranjang':
        ...
        break;
      case 'toko-terdekat':
        console.log('📍 Fallback: Navigate to nearby stores');
        break;
      case 'tracking-kurir':
        console.log('🚚 Fallback: Navigate to courier tracking');
        break;
      default:
  ...
  // ✅ NEW: Generate location-based deep links
  generateNearbyStoresLink(): string {
    return `ecommerceapp://toko-terdekat`;
  }

  generateCourierTrackingLink(): string {
    return `ecommerceapp://tracking-kurir`;
  }

  generateShippingCalculationLink(): string {
    return `ecommerceapp://hitung-ongkir`;
  }
  ...
  // ✅ NEW: Test location deep links
  async testLocationDeepLinks(): Promise<{ [key: string]: boolean }> {
    const results: { [key: string]: boolean } = {};
    
    try {
      const nearbyStoresUrl = this.generateNearbyStoresLink();
      const courierTrackingUrl = this.generateCourierTrackingLink();
      const shippingUrl = this.generateShippingCalculationLink();
      
      results.nearbyStores = await this.canOpenURL(nearbyStoresUrl);
      results.courierTracking = await this.canOpenURL(courierTrackingUrl);
      results.shippingCalculation = await this.canOpenURL(shippingUrl);
      
      console.log('🧪 Location deep links test results:', results);
      return results;
    } catch (error) {
      console.error('❌ Location deep links test error:', error);
      return {
        nearbyStores: false,
  ...
  isProtectedRoute(route: string): boolean {
    const protectedRoutes = [
      'cart', 'checkout', 'keranjang', 'payment', 'address', 'add-to-cart',
      // ✅ NEW: Location features juga butuh auth
      'tracking-kurir', 'courier-tracking'
    ];
  ...
  // ✅ NEW: Get all available deep links untuk testing
  getAllDeepLinks(): { [key: string]: string } {
    return {
      home: 'ecommerceapp://home',
      product: 'ecommerceapp://produk/123',
      cart: 'ecommerceapp://keranjang',
      profile: 'ecommerceapp://profil/user123',
      wishlist: 'ecommerceapp://wishlist',
      stats: 'ecommerceapp://statistik',
      addToCart: 'ecommerceapp://add-to-cart/456',
      // Location features
      nearbyStores: 'ecommerceapp://toko-terdekat',
      courierTracking: 'ecommerceapp://tracking-kurir',
      shipping: 'ecommerceapp://hitung-ongkir'
    };
  }
}
